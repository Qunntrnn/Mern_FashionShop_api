...
const createOrder = async (req, res) => {
  try {
    ...
    // For PayPal payment, calculate USD prices
      ...
      try {
        ...
        if (cartId) {
          ...
          if (deletedCart) {
            ...
            console.log("Cart not found for deletion:", cartId);
          }
        } else {
          console.log("No cartId provided for deletion");
        }
        ...
        // Try to delete cart even if there was an error
        if (cartId) {
          try {
            console.log("Attempting to delete cart after error:", cartId);
            const deletedCart = await Cart.findByIdAndDelete(cartId);
            if (deletedCart) {
              console.log("Cart deleted successfully after error:", cartId);
            } else {
              console.log("Cart not found for deletion after error:", cartId);
            }
          } catch (deleteError) {
            console.error("Error deleting cart after order error:", deleteError);
          }
        }
        res.status(500).json({
          success: false,
